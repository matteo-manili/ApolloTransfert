...
public class UserSecurityAdvice implements MethodBeforeAdvice, AfterReturningAdvice {
     ...
     * Questo metodo viene chiamato prima di ogni salvataggio sul db di uno user
     */
    ...
    public void before(Method method, Object[] args, Object target) throws Throwable {
        ...
        if (ctx.getAuthentication() != null) {
            ...
            for (GrantedAuthority role : roles) {
                if (role.getAuthority().equals(Constants.ADMIN_ROLE) || role.getAuthority().equals(Constants.GEST_AUTISTA_ROLE) ) {
                    administrator = true;
                    break;
                }
            }
            ...
            if (!signupUser) {
                ...
                if (user.getId() != null && !user.getId().equals(currentUser.getId()) && !administrator) {
                    ...
                    if (!CollectionUtils.isEqualCollection(userRoles, authorizedRoles) 
                    		&& !SituazioneUserVenditore(userRoles) && !SituazioneUserVenditore(authorizedRoles)) {
    ...
    /**
     * Quando devo fare attivare il prifilo Venditore da User a Venditore, questo permette ad un utente non Admin di modificare il Role
     */
    private boolean SituazioneUserVenditore(Set<String> authorizedRoles){
    	if( authorizedRoles.size() == 2 && authorizedRoles.contains(Constants.USER_ROLE) && authorizedRoles.contains(Constants.VENDITORE_ROLE) ){
        	return true;
        }else{
        	return false;
        }
