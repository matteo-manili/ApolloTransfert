...
package com.apollon.webapp.util.email.dkim;
...
public class DKIMSigner {
	 ...
	 */
	public static MimeMessage MainIntestazioneMailDKIM_OLD(MimeMessage message) throws Exception{
		return new SMTPDKIMMessage(message, GetMeDKIMSigner());
	}
	
	/**
	 * non lo uso più vedere email tiket di mondoserver.it
	 */
	public static Message MainIntestazioneMailDKIM_OLD(Session session) throws Exception{
		return new SMTPDKIMMessage(session, GetMeDKIMSigner());
	}
	
	
	private static DKIMSigner GetMeDKIMSigner() throws Exception{
		String resourceName = "mail.apollotransfert.properties"; // could also be a constant
        ...
        DKIMSigner dkimSigner = new DKIMSigner(
				props.getProperty("mail.smtp.dkim.signingdomain"),
				props.getProperty("mail.smtp.dkim.selector"),
				props.getProperty("mail.smtp.dkim.privatekey"));
	
		dkimSigner.setIdentity(props.getProperty("mail.smtp.dkim.identity")+ "@" +props.getProperty("mail.smtp.dkim.signingdomain"));
		return dkimSigner;
	}
	

	private DKIMSigner(String signingDomain, String selector, String privkeyFilename) throws Exception {
		ClassLoader classLoader = getClass().getClassLoader();
		File privKeyFile = new File(classLoader.getResource( privkeyFilename ).getFile());     
		...
		RSAPrivateKey privKey = (RSAPrivateKey) keyFactory.generatePrivate(privSpec); //si ferma qua
