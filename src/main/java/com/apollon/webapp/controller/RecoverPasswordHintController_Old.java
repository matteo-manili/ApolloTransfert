...
public class RecoverPasswordHintController_Old {
    private final Log log = LogFactory.getLog(RecoverPasswordHintController_Old.class);
    ...
    throws Exception {
        ...
        try {
            ...
            msg.append("\n\nLogin at: ").append(ControllerUtil.getAppURL(request));
            message.setTo(user.getEmail());
            String subject = (ApplicationUtils.CheckAmbienteVenditore(request.getServletContext())) ?
            		'['+ text.getMessage("webapp.ncctransferonline.name") +"] "+ text.getMessage("user.passwordHint") :
            			'['+ text.getMessage("webapp.apollotransfert.name") +"] "+ text.getMessage("user.passwordHint");
            message.setSubject(subject);
            message.setText(msg.toString());
            mailEngine.sendSimple(message);
