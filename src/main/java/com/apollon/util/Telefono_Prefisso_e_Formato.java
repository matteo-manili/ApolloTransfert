...
/**
 * 
 * vedere: https://github.com/googlei18n/libphonenumber <br>
 * 
	Parsing Result (parseAndKeepRawInput()) <br>
	country_code	39 <br>
	national_number	3289126869 <br>
	extension	 <br>
	country_code_source	FROM_NUMBER_WITH_PLUS_SIGN <br>
	italian_leading_zero	false <br>
	raw_input	+393289126869 <br>
	
	Validation Results <br>
	Result from isPossibleNumber()	true <br>
	Result from isValidNumber()	true <br>
	Result from isValidNumberForRegion()	true <br>
	Phone Number region	IT <br>
	Result from getNumberType()	MOBILE <br>
		
	Formatting Results <br>
	E164 format	+393289126869 <br>
	Original format	+39 328 912 6869 <br>
	National format	328 912 6869 <br>
	International format	+39 328 912 6869 <br>
	
	PhoneNumberToTimeZonesMapper Results <br>
	Time zone(s)	[Europe/Rome, Europe/Vatican] <br>
	
	PhoneNumberToCarrierMapper Results <br>
	Carrier	Wind <br>
	
	 * @param numberPhone
	 */
public class Telefono_Prefisso_e_Formato {
	private static final Log log = LogFactory.getLog(Telefono_Prefisso_e_Formato.class);

	
	
	
	/**
	 * Per essere valido il numero telefono deve essere: +393289126869 OPPURE +39 328 912 6869 (ITALIA) <br>
	 * Per essere valido il numero telefono deve essere: +61406127695 OPPURE +61 406127695 (AUSTRALIA)
	 * @param numberPhone
	 * @return
	 */
	public static boolean NumeroTelefonicoValido(String numberPhone) {
		try {
			//String telefono = "+39 3289126869";
			String telefono = numberPhone;
			
			telefono = UtilString.RimuoviTuttiGliSpazi(telefono);
			String prefix = Telefono_Prefisso_e_Formato.Dammi_SiglaPaese_From_NumeroTelefono(telefono);
			//log.debug("prefix: "+prefix);
			PhoneNumberUtil phoneUtil = PhoneNumberUtil.getInstance();
			PhoneNumber NumberProto = phoneUtil.parse(telefono, prefix);
			...
			//log.debug("errore NumeroTelefonicoValido");
			return false;
		}
	}
	
	
	public static void Dammi_Formato_Telefonico(String numberPhone) {
		String Country = Dammi_SiglaPaese_From_NumeroTelefono(numberPhone);
		PhoneNumberUtil phoneUtil = PhoneNumberUtil.getInstance();
		try {
			PhoneNumber swissNumberProto = phoneUtil.parse(numberPhone, Country);
		  	// Produces "+41 44 668 18 00"
			log.debug(phoneUtil.format(swissNumberProto, PhoneNumberFormat.INTERNATIONAL));
			// Produces "044 668 18 00"
			log.debug(phoneUtil.format(swissNumberProto, PhoneNumberFormat.NATIONAL));
			// Produces "+41446681800"
			log.debug(phoneUtil.format(swissNumberProto, PhoneNumberFormat.E164));

		} catch (NumberParseException e) {
			log.debug("NumberParseException was thrown: " + e.toString());
		}
	}
	
	/**
	 * Formato in entrata: +393289126869
	 * @param numberPhone
	 * @return
	 */
	public static String Dammi_SiglaPaese_From_NumeroTelefono( String numberPhone ) {
		String Country = null;
		Map<String, String> country2phone = getAll();
		for(Map.Entry<String, String> entry : country2phone.entrySet()) {
		    if( numberPhone.contains( entry.getValue() )  ) {
	...
	private static String getPhone(String code) {
