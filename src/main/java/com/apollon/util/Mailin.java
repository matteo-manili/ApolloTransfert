 ...
 * Con la versione free posso inviare max 300 email al giorno, ma le email iviate contengono il logo di sendinblue, 
 * Quindi usare solo per inviare email pubblicitarie.
 * 
 * Tramite il metodo http.get_smtp_details() si prendono i parametri SMTP (nome utente, password e porta) ed inviare email come faccio solitamente.
 * ma se non si acquista la versione BASE 29€/mese di Mailin (https://account.sendinblue.com/pricing) alle email viene incorporato il logo di sendinblue
 * provocando alla email ricevuta in gmail la scritta: "Le immagini non sono visualizzate. Visualizza immagini sottostanti".
 *
 */
public class Mailin extends ApplicationUtils {

	
	public static void main(String[] args) {
		Mailin http = new Mailin("https://api.sendinblue.com/v2.0", "OUkRnILYJjGPWd9x", 5000);   //Optional parameter: Timeout in MS
        String str = http.get_account();
        
        // da qui ottengo i parametri SMTP (nome utente, password e porta) da utilizzare come già faccio con smtp2.apollotransfert.com e smtp.gmail.com
        System.out.println(http.get_smtp_details());
		
        // qui invio una email usando le API. Ma è meglio usare l'SMTP perché lo integro nell'applicazione
        Map < String, String > to = new HashMap < String, String > ();
		to.put("matteo.manili@gmail.com", "to whom!");
		Map < String, Object > data = new HashMap < String, Object > ();
		data.put("to", to);
		data.put("from", new String [] {ApplicationMessagesUtil.DammiMessageSource("email.info.apollotransfert", null),"ApolloTransfert"});
		data.put("subject", "My subject");
		data.put("html", "This is the <h1>HTML</h1>");

		str = http.send_email(data);
    ...
    private String do_request(String resource, String method, String input) throws Exception {
        ...
        if (!EMPTY_STRING.equals(input) && !"GET".equals(method)) {
            ...
            try {
                ...
                writer = new BufferedWriter(new OutputStreamWriter(outStream, Constants.ENCODING_UTF_8));
                writer.write(input);
                writer.flush();
            }
            //
